def hopscotch(board, size):
    ...
    for row in range(1, size):
        ...
        pre_is_dup = True if max(board[row - 1]) in board[row - 1][pre_idx_max + 1:] else False
        cur_is_dup = True if max(board[row    ]) in board[row    ][cur_idx_max + 1:] else False
